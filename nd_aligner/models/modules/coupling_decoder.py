...
class CouplingDecoderOutput(NamedTuple):
    ...
    loss: torch.Tensor  # stage-weighted scalar loss
...
class CouplingDecoder(nn.Module):
    ...
    Args:
        ...
        loss_decay_factor: Geometric factor for progressive stage losses.
        normalize_loss_weights: If True, stage weights sum to one.
        stage_loss_mode: Stage-loss weighting mode. Supported values are
            "geometric", "first_only", and "last_only".
    """
    ...
    def __init__(
        ...
        if loss_decay_factor < 0:
            raise ValueError(f"loss_decay_factor must be >= 0, got {loss_decay_factor}.")

        valid_stage_loss_modes = {
            "geometric",
            "first_only",
            "last_only",
        }
        if stage_loss_mode not in valid_stage_loss_modes:
            raise ValueError(
                "stage_loss_mode must be one of "
                + f"{sorted(valid_stage_loss_modes)}, got {stage_loss_mode!r}."
            )
    ...
    def forward(
        ...
        weights = self.get_stage_loss_weights(
            num_stages=len(mel_losses),
    ...
    def get_stage_loss_weights(
        self,
        num_stages: int,
        device: torch.device,
        dtype: torch.dtype,
    ) -> torch.Tensor:
        """Build reconstruction-loss weights for all decoder stages."""
        if num_stages <= 0:
            raise ValueError(f"num_stages must be > 0, got {num_stages}.")

        if self.stage_loss_mode == "geometric":
            weights = torch.tensor(
                [self.loss_decay_factor**i for i in range(num_stages)],
                device=device,
                dtype=dtype,
            )
        else:
            ...
            if self.stage_loss_mode == "first_only":
                weights[0] = 1.0
            elif self.stage_loss_mode == "last_only":
                weights[-1] = 1.0
            else:
                raise RuntimeError(f"Unsupported stage_loss_mode: {self.stage_loss_mode!r}.")
...
if __name__ == "__main__":  # test
    ...
    original_stage_loss_mode = decoder.stage_loss_mode
    expected_mode_weights = {
        "first_only": [1.0, 0.0, 0.0, 0.0],
        "last_only": [0.0, 0.0, 0.0, 1.0],
    }

    for stage_loss_mode, expected_values in expected_mode_weights.items():
        decoder.stage_loss_mode = stage_loss_mode
        mode_weights = decoder.get_stage_loss_weights(
            num_stages=num_refinement_steps + 1,
            device=torch.device(device),
            dtype=out.loss.dtype,
        )
        expected_mode_weight = torch.tensor(
            expected_values,
            device=device,
            dtype=out.loss.dtype,
        )
        assert torch.equal(mode_weights, expected_mode_weight), (
            stage_loss_mode,
            mode_weights.detach().cpu().tolist(),
        )

    decoder.stage_loss_mode = original_stage_loss_mode
