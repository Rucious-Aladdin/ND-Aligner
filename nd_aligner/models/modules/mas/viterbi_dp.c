 ...
 */
VITERBI_EXPORT int
viterbi_forward_backtrack_f32(const float* log_b,
                              const uint8_t* mask,
                              ...
                              float* viterbi_logp)
{
  ...
  for (int64_t b = 0; b < B; ++b) {
    ...
    memcpy(prev, initial_delta + batch_text_offset, text_size * sizeof(float));
    ...
    for (int64_t t = 1; t < speech_len; ++t) {
      const size_t time_offset = batch_dp_offset + (size_t)t * text_size;
      ...
      for (int64_t j = 0; j < T_text; ++j) {
        ...
        if (opt_sep_mask != NULL && j >= 2 &&
            opt_sep_mask[batch_text_offset + (size_t)(j - 1)] != 0) {
    ...
    for (int64_t t = speech_len - 1; t >= 0; --t) {
      ...
      if (t > 0) {
        const uint8_t move = moves[(size_t)t * text_size + (size_t)j_cur];
