...
__device__ __forceinline__ SequenceLengths recover_lengths(
    const bool* __restrict__ mask,
    const int batch_offset,
    const int speech_size,
    const int txt_size
) {
    SequenceLengths lengths{0, 0,};
...
__device__ __forceinline__ float logaddexp_pair(
    const float a,
    const float b,
...
__device__ __forceinline__ LocalSoftmaxNorm compute_local_softmax_norm(
    const float first,
    const float second,
    const float third,
...
__device__ __forceinline__ float local_softmax_weight(
    const float value,
    const LocalSoftmaxNorm norm,
    const float neg_large
) {
    if (
        value <= neg_large
        || norm.inv_exp_sum == 0.0f
    ) {
        return 0.0f;
    }
...
__global__ void log_alpha_forward_kernel(
    ...
    for (int speech_idx = 1; speech_idx < speech_size; ++speech_idx) {
        ...
        for (int txt_idx = threadIdx.x; txt_idx < txt_size; txt_idx += blockDim.x) {
            ...
            if (mask[index]) {
                ...
                float predecessor_sum =
                    logaddexp_pair(stay, advance, neg_large);
...
__global__ void log_beta_forward_kernel(
    const float* __restrict__ log_b,
    const bool* __restrict__ mask,
    const bool* __restrict__ opt_sep_mask,
    ...
    if (threadIdx.x == 0) {
        const SequenceLengths lengths =
            recover_lengths(mask, log_b_offset, speech_size, txt_size);
...
__global__ void log_alpha_backward_kernel(
    ...
    LocalSoftmaxNorm* successor_norm =
        reinterpret_cast<LocalSoftmaxNorm*>(
            shared_memory + 2 * txt_size
        );
    ...
    for (int speech_idx = spec_length - 2; speech_idx >= 0; --speech_idx) {
        ...
        for (int succ_txt_idx = threadIdx.x; succ_txt_idx < txt_length; succ_txt_idx += blockDim.x) {
            ...
            const float advance =
                succ_txt_idx >= 1
                    ? log_alpha[curr_row_offset + succ_txt_idx - 1] : neg_large;

            const bool skip_allowed =
                has_opt_sep && succ_txt_idx >= 2
                && opt_sep_mask[sep_offset + succ_txt_idx - 1];

            const float skip =
                skip_allowed
                    ? log_alpha[curr_row_offset + succ_txt_idx - 2] : neg_large;

            successor_norm[succ_txt_idx] =
                compute_local_softmax_norm(
                    stay,
                    advance,
                    skip,
                    skip_allowed,
...
__global__ void log_beta_backward_kernel(
    const float* __restrict__ grad_log_beta,
    const float* __restrict__ log_b,
    ...
    for (int speech_idx = 0; speech_idx < spec_length - 1; ++speech_idx) {
        ...
        for (int src_txt_idx = threadIdx.x; src_txt_idx < txt_length; src_txt_idx += blockDim.x) {
            ...
            if (mask[curr_idx]) {
                // stay: j -> j
                const float stay =
                    log_b[next_row_offset + src_txt_idx]
                    + log_beta[next_row_offset + src_txt_idx];

                // advance: j -> j + 1
                const float advance =
                    src_txt_idx + 1 < txt_length
                        ? (
                ...
                // skip: j -> j + 2
                const bool skip_allowed =
                    has_opt_sep && src_txt_idx + 2 < txt_length
                    && opt_sep_mask[sep_offset + src_txt_idx + 1];

                const float skip =
                    skip_allowed
                        ? (
                            log_b[next_row_offset + src_txt_idx + 2]
                            + log_beta[next_row_offset + src_txt_idx + 2]
                        ) : neg_large;

                norm = compute_local_softmax_norm(
                    stay,
                    advance,
                    skip,
                    skip_allowed,
        ...
        for (int dst_txt_idx = threadIdx.x; dst_txt_idx < txt_length; dst_txt_idx += blockDim.x) {
            ...
            if (mask[next_idx]) {
                ...
                // --------------------------------------------
                {
                    const float weight = local_softmax_weight(
                        destination_value,
                        source_norm[dst_txt_idx],
                        neg_large
                    );
                ...
                const bool skip_allowed =
                    has_opt_sep && dst_txt_idx >= 2
                    && opt_sep_mask[sep_offset + dst_txt_idx - 1];

                if (skip_allowed) {
                    const float weight = local_softmax_weight(
                        destination_value,
                        source_norm[dst_txt_idx - 2],
                        neg_large
                    );
            ...
            next_delta[dst_txt_idx] =
                mask[next_idx] ? grad_log_beta[next_idx] + contribution : 0.0f;
        }
        __syncthreads();
...
torch::Tensor log_alpha_backward_cuda(
    ...
    const size_t shared_memory_bytes =
        2 * static_cast<size_t>(txt_size) * sizeof(float)
