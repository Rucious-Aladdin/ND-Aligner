...
from nd_aligner.config.ndaligner.data_config import DataConfig, LossConfigs, TrainConfigs
from nd_aligner.config.ndaligner.training_module_config import NDAlignerTrainingModuleConfigs
from nd_aligner.config.utils.io import load_config, save_config
from nd_aligner.data.data_types import LossValues, TrainBatch
from nd_aligner.data.datafactory import DataFactory
from nd_aligner.data.quantile_bucket_sampler import QuantileDurationBatchSampler
from nd_aligner.logger.timit_logger import NDAlignerTimitLogger
from nd_aligner.logger.utils.plot_alignment import plot_alignment
from nd_aligner.logger.utils.plot_spectrogram import plot_spectrogram
from nd_aligner.models.ndaligner import (
    AlignerForward,
...
from nd_aligner.train.utils.anneal import get_linear_anneal_weight
from nd_aligner.train.utils.checkpoint_manager import CheckpointManager
from nd_aligner.utils.set_seed import set_seed
...
class NDAlignerTrainer(
    ...
    def on_fit_start(self) -> None:
        ...
        if self.data_config.extra_exp.train_time_eval_logging:
            ...
            timit_val_benchmarker = TIMITBenchMarker(
                root_dir=self.data_config.extra_exp.timit_val_root_dir,
            ...
            self.train_time_eval_logger = NDAlignerTimitLogger(
                ...
                timit_benchmarker=timit_val_benchmarker,
    ...
    def on_training_end(self, epoch: int, step: int):
        if self.train_time_eval_logger is not None:
            ...
            self.train_time_eval_logger = NDAlignerTimitLogger(
                base_dir=str(Path(self.data_config.extra_exp.base_dir) / "confirm"),
                experiment_name=self.data_config.extra_exp.exp_name,
                exp_variant=self.data_config.extra_exp.exp_variant,
                timit_benchmarker=timit_test_benchmarker,
                max_test_samples=None,
            )
