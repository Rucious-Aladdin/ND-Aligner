# https://github.com/MontrealCorpusTools/mfa-models/blob/main/scripts/alignment_benchmarks/data_prep/create_buckeye_benchmark.py
...
def load_file(path: Path, max_time):  # type: ignore
    begin = 0
    data = []
...
def mid_point(interval):  # type: ignore
    return interval.start + ((interval.end - interval.start) / 2)


def correct_phones(word_intervals, phone_intervals):  # type: ignore
    new_phone_intervals = []
...
def construct_phrases(word_intervals, max_time):  # type: ignore
    data = []
    cur_utt = []
    silence_padding = 0.2
...
def parse_files(
    ...
    speaker = speaker_pattern.search(file_name).group("speaker")  # type: ignore
