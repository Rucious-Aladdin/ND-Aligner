...
class BaseTrainer(ABC, Generic[T_DataConfig, T_ModelConfig]):
    ...
    def _setup_run_dir(self) -> str:
        if self.train_cfg.continue_dir:
            ...
            run_name = f"{self.train_cfg.run_name}_{timestamp}"
            run_dir = os.path.join(self.train_cfg.log_dir, run_name)
            print(f"📂  Creating new run directory: [ {run_dir} ]")

        os.makedirs(run_dir, exist_ok=True)
        save_config(self.data_config, os.path.join(run_dir, "data_config.json"))
        save_config(self.model_config, os.path.join(run_dir, "model_config.json"))
        print(f"📄  Configurations backed up for reproducibility.")
        return run_dir
