...
from tts.models.layers.convolution import ConformerConvModule
from tts.models.layers.feed_forward import FeedForwardModule
from tts.models.layers.linear import Linear
from tts.models.layers.residual_connection import ResidualConnectionModule
from tts.models.utils.positional_encoding import RelPositionalEncoding
...
class ConformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_cond and cond is not None:
            if cond.dim() == 2:
                # global condition: (B, C_cond) -> (B, 1, H)
                condition = self.cond_proj(cond).unsqueeze(1)
            elif cond.dim() == 3:
                # frame-level condition: (B, T, C_cond) -> (B, T, H)
                condition = self.cond_proj(cond)
            else:
                raise ValueError(
                    f"cond must have shape (B, C_cond) or (B, T, C_cond), "
...
class ConformerBackbone(nn.Module):
    ...
    Inputs: x, t, cond, mask
        - **x** (batch, time, dim): Tensor containing input vector.
        - **t** (batch): Diffusion time step or noise level.
        - **cond** (batch, cond_in_channels): Tensor containing condition vectors.
        - **mask** (batch, seq_length) or (batch, 1, seq_length): Mask for padded input.
    ...
    def __init__(
        ...
        # noise-level / diffusion-time embedding -> encoder_dim
        self.time_pos_emb = GaussianFourierProjection(encoder_dim)
    ...
    def forward(
        ...
        """
        Forward propagate an `x` for encoder training.

        Args:
            x (torch.FloatTensor): A input sequence passed to encoder. Typically for inputs this will be a padded
                `FloatTensor` of size ``(batch, seq_length, dimension)``.
            t (torch.FloatTensor or float): Diffusion time step or noise level. If float is given, it will be converted
                to a `FloatTensor` of size ``(batch,)``.
            cond (torch.FloatTensor): Speaker or emotion embedding.
            mask (torch.BoolTensor, optional): Mask for padded input. ``(batch, seq_length)`` or ``(batch, 1, seq_length)``.
        Returns:
            torch.FloatTensor
        ...
        """
        outputs = self.input_projection(x)
        ...
        if not torch.is_tensor(t):
            ...
            t = t.to(device=outputs.device, dtype=outputs.dtype)
            if t.dim() == 0:
                t = t.expand(B)
            else:
                t = t.view(B)

        t_emb = self.time_mlp(self.time_pos_emb(t))  # (B, encoder_dim)
        outputs = outputs + t_emb.unsqueeze(1)  # (B, T, encoder_dim)
